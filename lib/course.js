...
async function getPage(course_id, section, page) {
  const url = `https://cdn.rawgit.com/w3b3d3v/buildspace-projects/main/${course_id}/ptBR/${section}/${page.file}`
...
async function getCourseLessons(course) {
  ...
  return await Promise.all(
    lessons.map(async (l) => {
      return { ...l, markdown: await getPage(course.id, l.section, l.lesson.file) }
    })
  )
}
