...
function filterLanguage(course, language) {
  let sections = course.sections //TODO: Remove this after migration of metadata from all courses to the new multi-language structure
  if (course.metadata && course.metadata[language] && course.metadata[language].sections) {
    sections = course.metadata[language].sections
  }
  return { ...course, sections }
}

export async function getCourse(course_id, language = 'en') {
  const docRef = doc(db, 'courses', course_id)
  const courseDoc = await getDoc(docRef)
  const course = { id: course_id, ...courseDoc.data() }
  const filteredCourse = filterLanguage(course, language)
  const lessons = await getCourseLessons(course)
  return { ...filteredCourse, lessons }
}
