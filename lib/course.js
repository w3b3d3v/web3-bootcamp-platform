...
function normalizeSections(course, language) {
  let sections = course.sections[language] || course.sections;
  return { ...course, sections };
}

export async function getCourse(course_id, language) {
  const docRef = doc(db, 'courses', course_id)
  const courseDoc = await getDoc(docRef)
  const course = { id: course_id, ...courseDoc.data() }
  const normalizedCourse = normalizeSections(course, language);
  const lessons = await getCourseLessons(normalizedCourse)
  return { ...normalizedCourse, lessons }
}
