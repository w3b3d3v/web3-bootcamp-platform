...
const Tabs = (course) => {
  // console.log('course sections', course.course)
  const getCourse = course.course
  // map sections from course to tabs and show the lessons
  ...
  const handleTabChange = (section) => {
    ...
    setActiveSection(sections.find((s) => s.section === section))
    setActiveLesson(sections.find((s) => s.section === section).lessons[0])
        ...
        {sections.map((section) => {
          return (
            <ul
              ...
              onClick={() => handleTabChange(section.section)}
            >
