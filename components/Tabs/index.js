    ...
    <div className="flex flex-col">
      <div className="flex flex-row gap-8">
        {Object.keys(getCourse?.sections).sort().map((section) => {
          return (
            <a href={`#${section}`} key={section} onClick={() => toggle(section)} className={`item flex-grow px-4 font-bold py-2 hover:no-underline rounded shadow-lg text-black-100 dark:text-white-100 ${activeTab === section ? 'bg-gray-200   dark:bg-black-100 dark:text-primary-100' : 'bg-white-100 dark:bg-black-200'}`}
            >
