...
contract W3DBootcamp is ERC721URIStorage, AccessControl {
  ...
  mapping(bytes32 => mapping(bytes32 => uint)) private cohorts;
  ...
  function mintCertificate(string memory cohort, string memory course, address studentAddress) external onlyMinter() {
    ...
    _mintCertificate(cohort, course, encoded_course, course_description, studentAddress);
  }
  ...
  function _mintCertificate (
    ...
    cohorts[encoded_course][encoded_cohort] +=1;
    uint256 newStudentID = cohorts[encoded_course][encoded_cohort];
